         ...
         /*   else if (!isVND()) {
                if (p.contains(".")) {
                    ...
                    if (TextUtils.isEmpty(amounts[0])) {
                        ...
                        newText = UIModel.getInstance().getDotMoney(amount);
                        caretPoint += (newText.length() - beforeText.length());
                    }
                } else if (p.length() > length || (p.startsWith("0") && p.length() >= 2 && !p.startsWith("0."))) {
                    newText = this.beforeText;
                } else {
                    newText = UIModel.getInstance().getDotMoneyHasCcy(p, currency);
                    caretPoint += (newText.length() - beforeText.length());
                }
            } */
            else if (!p.startsWith("0")) {
                if (p.contains(".") || p.length() > length) {
                    newText = this.beforeText;
                } else if (TextUtils.isEmpty(currency)) {
                    newText = UIModel.getInstance().getDotMoney(p);
                    caretPoint += (newText.length() - beforeText.length());
                } else {
                    newText = UIModel.getInstance().getDotMoneyHasCcy(p, currency);
                    caretPoint += (newText.length() - beforeText.length());
